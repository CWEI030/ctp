...
预分配的账户独立队列。测试覆盖 1 至 5 个账户，并验证一个账户队列溢出不会阻塞或污染其他
账户。`engine --mode live` 仍只校验配置后明确退出，不连接 SimNow；多账户登录、策略、报单、
撤单和恢复均未实现。CTP 官方 SDK 和账号凭据都不应提交到 Git。
...
成功时会生成 `build/ctp_client`，CTest 应报告 5 个测试全部通过。这里测试的是配置、
离线行情分发、旧行情与交易客户端状态、超时及资源清理，不需要 SimNow 账号，也不会连接
SimNow。
...
`0 tests failed out of 5` 只表示五组离线自动化测试通过，不表示真实账号已经登录成功。
