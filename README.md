...
`benchmark evidence` 会在 `evidence/performance/` 生成可提交的逐纳秒无损直方图、原始 CSV 摘要、CPU/队列时间序列、环境快照和运行索引。它要求每账户四段链路各至少 10000 个样本，用于证明原始结果可校验、统计可重算，并给出当前测量条件下的延迟、吞吐、抖动、CPU、积压和丢弃结论；不据此声明生产容量或 SLA。完整原始 CSV 保留在被 Git 忽略的 `runtime/` 目录。
