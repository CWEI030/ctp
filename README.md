...
> 当前定位：核心功能已完成离线自动化验证，并提交了可复现的短时高样本性能证据；至少四个真实 SimNow 账户的联合在线验收仍未完成。性能数据只代表记录的离线条件，不代表生产容量或 SLA，本项目不能据此直接用于实盘。
...
- 撤单请求同步未被 CTP 接受时会回滚本地撤单状态、去重标志和本次占用的每日撤单额度，后续人工或自动撤单可再次调用 CTP；成功发送后的重复撤单仍只调用一次。
- 开仓成交后可生成相反方向的平仓意图；入场单超时只撤一次，平仓单超时可在限定次数内撤单重报，次数耗尽后冻结账户。
- 账户重连后重新查询订单、成交、持仓和资金；回报队列溢出时也会由账户线程主动执行同样的四项核对，全部收敛前保持冻结。
- 认证、登录和四项查询记录脱敏后的 `ErrorID/ErrorMsg` 与恢复转换。退出时逐账户输出 `[recovery]` 摘要，其中 `failed_phase` 标明冻结前的位置；`initialized=0` 表示账户未完成本地初始化。`ErrorID=0` 不排除本地请求失败、缺失响应或协议异常，应结合 `failure` 判断，不能仅凭账户冻结推断密码错误。
- 新轨迹采用 Trace v3，仍可读取 v1/v2；诊断只占用非关键日志容量。错误文本按当前账户非空的 BrokerID、UserID、Password、AppID、AuthCode 配置值脱敏，过滤控制字符和 CSV 分隔符，最多保留 80 字节，不做柜台字符编码转换。
...
- 当前性能证据不是容量认证；短时离线数据不能代表真实 CTP 网络、柜台或生产机器性能。
...
| `trading` | 订单、成交、持仓、风控、撤单同步失败重试、平仓、重报和回报竞态 |
...
历史提交证据位于 `evidence/performance/evidence-20260911T141213/`，三份运行清单和采集前后环境快照均记录冻结基线 `e5bf57392dacd42abed821817ce7ca323ed911be`。该目录包含 1、2、4 逻辑账户的完整原始数据、报告与校验清单。后续风险计数与诊断功能已改变生产代码，该数据不能代表当前版本性能；当前源码需要重新采集高样本证据，离线性能冒烟不能替代它。
