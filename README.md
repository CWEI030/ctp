...
> 当前定位：核心功能已完成离线自动化验证，但尚未完成至少四个真实 SimNow 账户的联合在线验收，也尚未取得正式长时间性能矩阵数据。本项目不能据此直接用于实盘。
...
`benchmark full` 全部完成后还会在 `evidence/performance/` 生成可提交的逐纳秒无损直方图、原始 CSV 摘要、环境快照和矩阵索引。完整原始 CSV 仍保留在被 Git 忽略的 `runtime/` 目录；未完成的运行不会发布为正式证据。
