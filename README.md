...
加入了每账户风控、唯一报单引用、报单/撤单请求转换，以及订单和成交回调的有界队列接入。

上述第四批能力目前只由替身 CTP 接口完成离线闭环。`engine --mode live` 仍只校验配置后明确退出，
没有装配账户交易会话，也不会向 SimNow 报单；策略、自动平仓、断线恢复和性能验收仍未实现。
...
离线行情分发、订单/成交/持仓状态、风控、替身接口报撤单和回调接入、旧行情与交易客户端状态、
超时及资源清理，不需要 SimNow 账号，也不会连接 SimNow。
...
- 多账户 `engine` 已有配置、离线行情分发、订单/成交/持仓、风控和替身接口报撤单闭环，
  但这些模块尚未由 `main` 装配，也未经过真实 SimNow 报撤单与回报验证；
